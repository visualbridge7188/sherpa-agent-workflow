...
function duo_obituary_asset_version($skin_dir, $file){
	...
	return defined('DUO_OBITUARY_KBOARD_VERSION') ? DUO_OBITUARY_KBOARD_VERSION : '1.0.6';
}
...
function duo_obituary_encode_payload($payload){
	...
	return '<div class="duo-obituary-payload" title="' . esc_attr(base64_encode($json)) . '"></div>';
}
...
function duo_obituary_add_query_joins($from, $board_id, $list){
	...
	if(strpos($from, 'duo_deceased_name') === false){
		$joins[] = "LEFT JOIN `{$option_table}` AS duo_deceased_name ON `{$content_table}`.`uid`=duo_deceased_name.`content_uid` AND duo_deceased_name.`option_key`='deceased_name'";
	}
	if(strpos($from, 'duo_chief_mourner') === false){
		$joins[] = "LEFT JOIN `{$option_table}` AS duo_chief_mourner ON `{$content_table}`.`uid`=duo_chief_mourner.`content_uid` AND duo_chief_mourner.`option_key`='chief_mourner'";
	}
...
function duo_obituary_query_where($where, $board_id, $list){
	...
	$keyword = isset($_GET['duo_obituary_keyword']) ? sanitize_text_field(wp_unslash($_GET['duo_obituary_keyword'])) : '';
	$target = isset($_GET['duo_obituary_target']) ? sanitize_key(wp_unslash($_GET['duo_obituary_target'])) : 'deceased_mourner';

	if($keyword !== ''){
		$like = '%' . $wpdb->esc_like($keyword) . '%';
		if($target === 'deceased_name'){
			$conditions[] = $wpdb->prepare('duo_deceased_name.`option_value` LIKE %s', $like);
		}
		else if($target === 'chief_mourner'){
			$conditions[] = $wpdb->prepare('duo_chief_mourner.`option_value` LIKE %s', $like);
		}
		else{
			$conditions[] = $wpdb->prepare('(duo_deceased_name.`option_value` LIKE %s OR duo_chief_mourner.`option_value` LIKE %s)', $like, $like);
		}
	}
